...
public class SingularValueDecomposition implements java.io.Serializable {
    ...
    /**
     * Created by usa on 10/25/14.
     */
    public Matrix inverse() {
        Matrix uT = new Matrix(U).transpose();
        Matrix v = new Matrix(V);
        Solver solver = new Solver(s, uT, v);
    ...
    private static class Solver {
        ...
        private Solver(final double[] singularValues, final Matrix uT,
                      final Matrix v) {
           double[][] suT = uT.getArrayCopy();
           ...
           pseudoInverse = v.times(new Matrix(suT)); //CHECK!?
       }
